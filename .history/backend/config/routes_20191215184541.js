...
router.get('/token/new', function(req, res) {
  const token = jsonWebTokenController.generate({ username: 'Bruno Martins' })

  res.send(jsonWebTokenController.decode(token).payload)
})
