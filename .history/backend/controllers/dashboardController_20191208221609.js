...
const listRoom = async function(req, res) {
  ...
  await db('Rooms')
    .select('Datasheet.*', 'Ticket_Options.ticket_option_designation')
    ...
    .where('Type_Of_User.type', '=', 'Entidade').andWhere('Companies.uuid_company', '=', req.userLogged.company.uuid_company)

    .then(data => {
      return res.send({data, userLogged: req.userLogged})
    })
