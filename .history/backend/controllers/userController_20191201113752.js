...
const register = async function(req, res) {
  ...
  if (Object.keys(errors).length > 0) return res.render('index', { title: 'SyncReady', page: 'main/register', errors , predata } )
  else {
    await saveUser(req)
      .catch(_ => {
        errors.errorOnInsert = messageErrorOnInsert
...
const auth = function(req, res) {
  ...
  if (Object.keys(errors).length > 0) return res.render('index', { title: 'SyncReady', page: 'main/login', errors, messageErrorOnInsert: null })
  else login(email, password, req, res) 
}

const saveUser = async function(req) {
  const { fullname, address, email, contacto, cc, username, password } = req.body
  return db('Userss').insert({
    nickname: username,
    fullname,
    address, 
    email,
...
const post = async function(req, res) {
  ...
  if(Object.keys(errors).length > 0) return res.send(403).send(errors)
  await saveUser(req)
