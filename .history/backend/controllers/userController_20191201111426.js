...
const register = async function(req, res) {
  ...
  if (Object.keys(errors).length > 0) {
    ...
    const { fullname, address, email, contacto, cc, username, password, repeatpassword } = predata

    await db('Users').insert({
      nickname,
      fullname: const { fullname, address, email, contacto, cc, username, password, repeatpassword } = req.body,
      address, 
      email,
      telephone: contacto,
      citizencard: cc,
